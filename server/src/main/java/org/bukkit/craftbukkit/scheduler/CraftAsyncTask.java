...
class CraftAsyncTask extends CraftTask {
	...
	public void run() {
		...
		try {
			...
			thrown = t;
			throw new RuntimeException(String.format("Plugin %s generated an exception while executing task %s", getOwner().getDescription().getFullName(), getTaskId()), thrown);
		} finally {
			...
			synchronized (workers) {
				try {
					...
					if (!removed)
						throw new IllegalStateException(String.format("Unable to remove worker %s on task %s for %s", thread.getName(), getTaskId(), getOwner().getDescription().getFullName()), thrown); // We don't want to lose the original exception, if any
				} finally {
